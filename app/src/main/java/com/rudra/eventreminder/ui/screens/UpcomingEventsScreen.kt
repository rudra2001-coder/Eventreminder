...
@Composable
fun UpcomingEventsScreen(viewModel: EventViewModel, onNavigateBack: () -> Unit) {
    val upcomingEvents by viewModel.events.collectAsState(initial = emptyList())

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "Upcoming Events", style = MaterialTheme.typography.headlineMedium.copy(fontWeight = FontWeight.Bold)) },
                navigationIcon = {
